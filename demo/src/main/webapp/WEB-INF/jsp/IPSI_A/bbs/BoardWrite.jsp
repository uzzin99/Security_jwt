...
$().ready(function() {
	...
	$(document).on('click', 'button.goSave', function() {
		...
		if("${boardConf.USE_SECRET_YN}" == "Y"){
			if(!$("#sendForm [name=SECRET_YN]").is(":checked")){
				alert("공개여부를 선택하세요.");
	            return;
	        }else{
	        	$("[name=SECRET_YN]").val($("[name=SECRET_YN]:checked").val());
	        }
		}
    ...
    <div class="applyTable qna_write_table">
	    ...
	    <div class="lineTop_tbArea">
			<table class="lineTop_tbL2">
				...
				<c:if test="${boardConf.USE_SECRET_YN eq 'Y'}">
					<tr>
						<th scope="row"><span class="mark_need">[필수]</span> 공개여부</th>
						<td class="txtL">
							<fieldset class="optionGrp">
								<label class="inputBox">
									<input type="radio" value="N" name="SECRET_YN">
									<span class="data">공개</span>
								</label>
	
								<label class="inputBox">
									<input type="radio" value="Y" name="SECRET_YN">
									<span class="data">비공개</span>
								</label>
							</fieldset>
						</td>
						<th scope="row"><span class="mark_need">[필수]</span> 확인번호</th>
						<td class="txtL">
							<input class="inputBase" type="password" maxlength="4" title="확인번호 입력" name="PASSWD" autocomplete="off" oninput="maxLengthCheck(this)" data-constr="num">
						</td>
					</tr>
				</c:if>
				<c:if test="${boardConf.USE_SECRET_YN ne 'Y'}">
					<th scope="row"><span class="mark_need">[필수]</span> 확인번호</th>
						<td colspan="3" class="txtL">
