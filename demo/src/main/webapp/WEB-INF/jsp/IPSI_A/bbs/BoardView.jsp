...
// 		$("#sendForm").attr("action", "${BASE_PATH}/cms/FR_CON/index.do");
// 	    $("#sendForm").submit();
		location.href = "${(not empty BASE_PATH && BASE_PATH != '')?'/':''}${BASE_PATH}/cms/FR_CON/index.do?"+$("#sendForm").serialize();
	});
...
<%-- 			                 	<button type="button" class="btn_preview" data-boardseq="${list.BOARD_SEQ}" data-siteno="${list.SITE_NO }" data-bbsseq="${list.BBS_SEQ }" data-fileseq="${list.FILE_SEQ }">미리보기</button> --%>
	...
	<div class="btn_Area btn_Area_R">
		<c:if test="${boardConf.USE_REPLY_YN eq 'Y' && AUTH_REPLY_PASS && boardItem.REPLY_BBS_YN eq 'N' }">
			<button type="button" class="btnC btnOrange btn_reply"><span>답글</span></button>
		</c:if>
